...
import sys, os, time, gc, math, random, warnings
...
if __name__ == "__main__":
    ...
    print(f"\n  FY2 物理引导搜索 (15000点)...")
    t0 = time.time()
    fy2_x, fy2_T, fy2_feas, fy2_eff = physics_guided_search(FY2_0, 1400.0, n_samples=15000, seed=42)
    print(f"  FY2: {fy2_feas} feas, {fy2_eff} eff | T_eff={fy2_T:.4f}s ({time.time()-t0:.1f}s)")
    if fy2_x:
        td, Ax, Ay, Az, feas = fy2_x
    ...
    sigma0 = 1.0
    print(f"  CMA-ES: sigma0={sigma0}, maxfevals=2000")

    try:
        es = cma.CMAEvolutionStrategy(x0_clipped, sigma0, {
            'bounds': [lb_array.tolist(), ub_array.tolist()],
            'maxfevals': 2000, 'seed': 42, 'verbose': -1,
            'CMA_diagonal': True, 'popsize': 24})
        ...
        while not es.stop():
            ...
            gen_count += 1
            if gen_count % 10 == 0 or gen_count == 1:
